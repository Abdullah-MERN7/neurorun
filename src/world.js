...
// Road deck & Ballast bed (Ballast is wide 14m bed, 100% covering railway corridor)
const roadDeckGeo = new THREE.BoxGeometry(8.8, 0.2, segLength);
...
const halfRoadGeo = new THREE.BoxGeometry(8.8, 0.2, segLength / 2);
const halfBallastGeo = new THREE.BoxGeometry(14.0, 0.25, segLength / 2);
const halfCurbGeo = new THREE.BoxGeometry(0.35, 0.35, segLength / 2);

// Curbs (ONLY used in Road zones)
const curbGeo = new THREE.BoxGeometry(0.35, 0.35, segLength);
...
// Side terrains: 160m width each side = 328m+ total terrain width across screen (NO empty void!)
...
const transitionPlateGeo = new THREE.BoxGeometry(14.0, 0.08, 1.5);
...
class WorldSegment {
  constructor(scene, segmentIndex) {
    ...
    // Pre-build cached standalone groups (NOT added to this.group yet!)
    ...
    // Apply initial zone (attaches ONLY the matching groups to this.group)
  ...
  buildRoadSurface() {
    ...
    // 4. Yellow Road Hazard Edges at X = ±4.3
  ...
  // -------------------------------------------------------------
  buildRailSurface() {
    // 1. Wide Crushed-Stone Ballast Bed (14m wide, warm granite stone ballast)
    const ballast = new THREE.Mesh(ballastBedGeo, ballastBedMat);
    ballast.position.set(0, -0.125, halfSeg);
    ballast.receiveShadow = true;
    this.railSurfaceGroup.add(ballast);

    // 2. Wide Side Railway Gravel Ground extending 160m to each horizon
    const sideL = new THREE.Mesh(sideGroundGeo, railGravelMat);
    sideL.rotation.x = -Math.PI / 2;
    sideL.position.set(-87.0, -0.125, halfSeg);
    ...
    sideR.position.set(87.0, -0.125, halfSeg);
    sideR.receiveShadow = true;
    this.railSurfaceGroup.add(sideR);

    // 3. Three Continuous Parallel Railway Tracks (Left, Center, Right)
    ...
    CONFIG.LANES.forEach((laneX) => {
      for (let i = 0; i < piecesPerSegment; i++) {
        ...
        trackPiece.position.set(laneX, 0.02, -i * pieceLength);
        this.railSurfaceGroup.add(trackPiece);
      }
    });

    // 4. Overhead Railway Catenary Infrastructure at Z = -20m
    const catenary = assetLoader.getModelClone('overhead_gantry');
    catenary.position.set(0, 0, halfSeg);
    ...
    sigR.position.set(6.2, 0, halfSeg - 4);
  ...
  buildTransitionInSurface() {
    ...
    // Yellow expansion joint transition plate at Z = -20
    const plate = new THREE.Mesh(transitionPlateGeo, transitionPlateMat);
    plate.position.set(0, 0.01, -segLength / 2);
    this.transInSurfaceGroup.add(plate);

    // 3 Railway Tracks start at Z = -20 and continue cleanly to Z = -40
    const pieceLength = 4.0;
    CONFIG.LANES.forEach((laneX) => {
      for (let i = 5; i < 10; i++) {
        const trackPiece = assetLoader.getModelClone('track_rail');
    ...
    // Overhead Railway Corridor Entrance Gantry at Z = -18m
    const entranceGantry = assetLoader.getModelClone('overhead_gantry');
    entranceGantry.position.set(0, 0, -18);
    ...
    amberAspect.position.set(6.2, 3.2, -17.8);
    this.transInSurfaceGroup.add(amberAspect);
  }

  // -------------------------------------------------------------
  // TRANSITION FROM RAIL (Tracks end at crossing plate -> ballast tapers -> road resumes)
  // -------------------------------------------------------------
  buildTransitionOutSurface() {
    // Ballast bed on first half (Z: 0 to -20) - 14m wide pure ballast
    const ballastHalf = new THREE.Mesh(halfBallastGeo, ballastBedMat);
    ...
    // Road deck on second half (Z: -20 to -40)
    const roadHalf = new THREE.Mesh(halfRoadGeo, roadDeckMat);
    roadHalf.position.set(0, -0.1, -3 * segLength / 4);
    roadHalf.receiveShadow = true;
    this.transOutSurfaceGroup.add(roadHalf);

    // Road Curbs ONLY on second half (Z: -20 to -40)
    const curbL2 = new THREE.Mesh(halfCurbGeo, roadCurbMat);
    curbL2.position.set(-4.45, 0.08, -3 * segLength / 4);
    this.transOutSurfaceGroup.add(curbL2);

    const curbR2 = new THREE.Mesh(halfCurbGeo, roadCurbMat);
    curbR2.position.set(4.45, 0.08, -3 * segLength / 4);
    this.transOutSurfaceGroup.add(curbR2);

    // Yellow transition plate at Z = -20
    const plate = new THREE.Mesh(transitionPlateGeo, transitionPlateMat);
    plate.position.set(0, 0.01, -segLength / 2);
    this.transOutSurfaceGroup.add(plate);

    // 3 Railway Tracks continue from previous railway segment and end cleanly at Z = -20
    const pieceLength = 4.0;
    CONFIG.LANES.forEach((laneX) => {
      for (let i = 0; i < 5; i++) {
        const trackPiece = assetLoader.getModelClone('track_rail');
    ...
    // Exit Catenary Termination Gantry at Z = -18m
    const exitGantry = assetLoader.getModelClone('overhead_gantry');
    exitGantry.position.set(0, 0, -18);
  ...
  prebuildAllScenery() {
    const segZ = halfSeg;
    const sideX = 84.4; // Half-width offset for 160m wide terrain planes (spans 4.4m to 164.4m)

    // --- 1. ZONE: CITY ---
    const cityGroundL = new THREE.Mesh(sideGroundGeo, cityGroundMat);
    cityGroundL.rotation.x = -Math.PI / 2;
    cityGroundL.position.set(-sideX, -0.05, segZ);
    ...
    cityGroundR.position.set(sideX, -0.05, segZ);
    ...
    indGroundL.position.set(-sideX, -0.05, segZ);
    ...
    indGroundR.position.set(sideX, -0.05, segZ);
    ...
    [-14, -7, 0, 7, 14].forEach((zOff, idx) => {
      ...
      rockL.position.set(-8.5 - Math.abs(zOff) * 0.25, 0, segZ + zOff);
      rockL.scale.setScalar(scaleL);
      rockL.rotation.y = (zOff * 0.45);
      this.desertGroup.add(rockL);

      // Mid-distance left mesa rock
      if (idx % 2 === 0) {
      ...
      rockR.position.set(8.5 + Math.abs(zOff) * 0.25, 0, segZ + zOff + 3);
      rockR.scale.setScalar(scaleR);
      rockR.rotation.y = -(zOff * 0.4);
      this.desertGroup.add(rockR);

      // Mid-distance right mesa rock
      if (idx % 2 === 1) {
    ...
    [-14, -6, 2, 10].forEach((zOff) => {
      ...
      bushL.position.set(-6.4, 0, segZ + zOff + 2);
      ...
      bushR.position.set(6.4, 0, segZ + zOff - 2);
   ...
   * Detaches and removes ALL children from this.group.
   * Hides all unused cached groups to guarantee zero scenery leaks between biomes.
   * Attaches ONLY the surface and scenery belonging to nextZone.
   */
  setZone(nextZone) {
    this.currentZone = nextZone;

    // 1. Physically REMOVE all existing children from this.group
    while (this.group.children.length > 0) {
      this.group.remove(this.group.children[0]);
    }

    // 2. Hide ALL cached groups to guarantee no leaking
    ...
    const isRoad = (
      nextZone === ZoneType.CITY_ROAD ||
      nextZone === ZoneType.INDUSTRIAL ||
      nextZone === ZoneType.DESERT ||
      nextZone === ZoneType.COAST ||
      nextZone === ZoneType.GREEN
    );

    // 3. Attach ONLY the designated surface and scenery
    if (isRoad) {
